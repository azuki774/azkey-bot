...
def follow_command(limit):
    ...
    try:
        ...
        # Log results
        logger.info(
            f"action=follow_complete total_followers={result['total_followers']} "
        ...
        for failed in result["failed_follows"]:
            logger.warning(
                f'action=follow_failed user_id={failed["follow_id"]} error="{failed["error"]}"'
            )

        # Log final result summary
        if result["users_to_follow_back"] > 0:
            logger.info(
                f'action=follow_summary message="Follow back completed" '
...
def dakoku_command(user_id):
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f'action=username_retrieved user_id={user_id} username="{username}"'
            )
        except Exception as e:
        ...
        if result.get("already_checked_in", False):
            logger.info(
                f'action=already_checked_in user_id={user_id} username="{username}" '
...
def timeline_command(limit, until_id, verbose):
    ...
    try:
        ...
        logger.info(
            f'action=timeline_retrieved post_count={len(timeline)} message="Retrieved timeline posts"'
        )
        ...
        if timeline:
            ...
            logger.info(
                f"action=timeline_pagination last_post_id={last_post_id} "
...
def check_command():
    ...
    try:
        ...
        logger.info(
            f'action=keyword_match_complete matching_posts={len(matching_posts)} message="Keyword matching completed"'
        )
        ...
        for i, post in enumerate(matching_posts, 1):
            ...
            logger.info(
                f"action=process_post post_number={i} user_id={user_id} "
            ...
            try:
                ...
                if result.get("already_checked_in", False):
                    ...
                    already_checked_in.append(
                        {"user_id": user_id, "username": username, "note_id": post_id}
                    )
                else:
                    logger.info(
                        f'action=checkin_success user_id={user_id} username="{username}" '
                    ...
                    try:
                        ...
                        logger.info(
                            f"action=reaction_added post_id={post_id} reaction=👍"
                        )
                ...
                logger.error(
                    f'action=checkin_failed user_id={user_id} username="{username}" '
        ...
        # 結果サマリー
        logger.info(
            f"action=check_complete matching_posts={len(matching_posts)} "
